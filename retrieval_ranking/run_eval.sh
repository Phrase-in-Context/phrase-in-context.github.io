...
function run() {
  ...
  OUTPUT_DIR=../results/${DATASET}/${SUBSET}/ranking
  mkdir -p ${OUTPUT_DIR}
  ...
  echo "${params[@]}"
  nohup python eval.py "${params[@]}" > ${OUTPUT_DIR}/logs/eval_logs_${DATASET}_${EXTRACTOR}_${NGRAMS_MIN}_${NGRAMS_MAX}_${SCORER}_${SCORE_TYPE}_${ORACLE}_${CONTEXTUAL}_${CONTEXT_WINDOW}_${GPU}_ver${CHECKPOINT}.txt &
}
...
#run ${dataset} ${data_subset} ${extractor} 2 3 sentbert sentbert-ppdb 128 ${oracle} ${noun_chunk_filter} ${contextual} -1 1
...
run ${dataset} ${data_subset} ${extractor} 2 3 transformers bert-base-uncased ${max_seq_len} ${oracle} ${noun_chunk_filter} ${contextual} -1 3
