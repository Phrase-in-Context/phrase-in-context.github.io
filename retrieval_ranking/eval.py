...
def run(args):
    """ """
    # Load data
    data = load_dataset(args.dataset, args.data_subset)["test"]
    ...
    # Export results
    outdir = os.path.join(args.outdir, datetime.datetime.now().strftime('%Y-%m-%d_%H-%M-%S'),
                          "{}_{}_{}_{}_{}_oracle-{}_{}{}".format(args.scorer, args.scorer_type, args.extractor,
                                                                 args.ngram_min, args.ngram_max, args.oracle_candidates,
                                                                 ("_contextual" if args.contextual else ""), args.context_window))

    _export_results(eval_results, run_results, outdir)
...
if __name__ == "__main__":
    ...
    parser.add_argument('--scorer', help='model name', choices=['transformers', 'sentbert', 'spanbert', 'use', 'simcse', 'densephrases'])
    parser.add_argument('--scorer_type', help='transformers type',
                        choices=['bert-base-uncased', 'bert-large-uncased', 'Sbert-base-nli-stsb-mean-tokens', 'span-bert-base-cased',
                                 'v5', 'simcse-bert-base-uncased', 'phrasebert', 'densephrases-multi-query-multi',], default="")
